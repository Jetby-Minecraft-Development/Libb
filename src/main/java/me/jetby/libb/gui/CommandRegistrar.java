...
public class CommandRegistrar extends BukkitCommand implements CommandExecutor {
    ...
    public CommandRegistrar(Libb plugin, String menuId, String commandName) {
        super(commandName);
        this.plugin = plugin;
        this.menuId = menuId;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        new ParsedGui(player, Libb.PARSED_GUIS.get(menuId), plugin).open(player);


        return true;
    }

    public static void createCommands(Libb plugin) {
        Map<String, List<String>> commands = new HashMap<>();
        Libb.PARSED_GUIS.forEach((id, gui) -> {
            commands.put(id, gui.command());
        });
